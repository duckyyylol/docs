...
### Setting up Spoofify
<Expandable title="Steps to set up Spoofify">
    <Steps>
    <Step title="Create a Spotify Developer Application">
        ...
        Navigate to https://developer.spotify.com/dashboard, choose `Create app`, and enter the following information:
    ...
    <Step title="Provide Application Credentials">
        ...
        Choose `View client secret`, then copy the `Client Secret` and provide it to Ducky.
    ...
    <Step title="First Login">
        Once everything is set up, it's time to log in for the first time! Head to the URL provided by Ducky and click `Log In` to authorize Spotify
    </Step>
</Steps>
</Expandable>

### Setting up the Twitch Bot
<Expandable title="Steps to set up the Twitch Bot" defaultOpen>
    <Steps>
        <Step stepNumber="0">
        ...
        <Step stepNumber="2" title="Add the bot to your channel">
            Join the bot account's Twitch chat (https://twitch.tv/popout/{username}/chat)

            Run `!join` using the channel you want the bot to join.
